...
- **Arturia plugins fail to capture a thumbnail** — `Delay TAPE-201`, `Efx REFRACT`, and `Tape MELLO-FI` consistently fail with `no_view` at ~8s (CaptureHelper's GUI-render window), across every scan attempt in the DB's history. Not caused by, or related to, the grouping/scan work above — these are single-component plugins grouping never touches. Not yet investigated; may need a longer render timeout, or may be a fundamental limitation of instantiating a license-gated plugin out-of-process without a real host.
